...
describe "usage scenarios" do
  ...
  #
  # TODO: check for return codes at the very least
  #
  ...
  before(:all) do
    ...
    if local_box_available?(BASEBOX) && !box_imported?(BASEBOX)
      system_cmd("vagrant box add #{BASEBOX} file://#{BUILD_DIR}/boxes/#{BASEBOX}.box")
    end
    # remove repos
    require 'fileutils'
  ...
  describe "developing application cookbooks" do
    it "clones an application cookbook via `git clone`" do
      system_cmd("cd #{REPO_DIR} && git clone https://github.com/tknerr/sample-application-cookbook.git")
    end
    it "installs gem dependencies via `bundle exec`" do
      system_cmd("cd #{APP_COOKBOOK_DIR} && bundle install")
    end
    it "runs the unit tests via `rake test`" do
      system_cmd("cd #{APP_COOKBOOK_DIR} && rake test")
    end
    it "runs the integration tests via `rake integration`" do
      system_cmd("cd #{APP_COOKBOOK_DIR} && rake integration")
    end
  end

  describe "managing infrastructure" do
    it "clones an infrastructure repo via `git clone`" do
      system_cmd("cd #{REPO_DIR} && git clone https://github.com/tknerr/sample-infrastructure-repo.git")
    end
    it "installs vagrant plugin dependencies via `vagrant plugin bundle`" do
      system_cmd("cd #{INFRA_REPO_DIR} && vagrant plugin bundle")
    end
    ...
    it "can destroy a VM via `vagrant destroy`" do
      system_cmd("cd #{INFRA_REPO_DIR} && vagrant destroy -f app_v2")
    end
  end
end